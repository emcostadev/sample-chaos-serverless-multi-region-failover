...
ADD_PRODUCT_URL = f"http://localhost:{API_GATEWAY_PORT}/restapis/{PRIMARY_API_ID}/dev/_user_request_/productApi"
    ...
    try:
        if induce:
            response = requests.post(
                CHAOS_FAULTS_URL, json=fault_payload, timeout=timeout
            )
        else:
...
def check_active_faults(expected_to_be_present_or_absent, present=True, timeout=5):
    try:
        response = requests.get(CHAOS_FAULTS_URL, timeout=timeout)
        response.raise_for_status()
...
def dynamodb_resource():
    return boto3.resource(
        "dynamodb", endpoint_url=AWS_ENDPOINT_URL, region_name=SERVICE_REGION
    )
...
def lambda_client():
    return boto3.client(
        "lambda", endpoint_url=AWS_ENDPOINT_URL, region_name=SERVICE_REGION
    )
...
def api_gateway_client():
    return boto3.client(
        "apigateway", endpoint_url=AWS_ENDPOINT_URL, region_name=SERVICE_REGION
    )


def get_primary_api_url(api_gateway_client):
    response = api_gateway_client.get_rest_apis()
    for api in response.get("items", []):
        if api.get("tags", {}).get("_custom_id_") == PRIMARY_API_ID:
            return (
                f"http://localhost:{API_GATEWAY_PORT}/restapis/"
                f"{api['id']}/dev/_user_request_/productApi"
            )
    pytest.fail(f"API Gateway com ID customizado '{PRIMARY_API_ID}' não encontrada.")
...
def test_dynamodb_outage_impacts_add_product(dynamodb_resource, api_gateway_client):
    ...
    add_product_url = get_primary_api_url(api_gateway_client)
    try:
        response_normal = requests.post(
            add_product_url, headers=headers, json=normal_data, timeout=30
        )
    ...
    try:
        response_outage = requests.post(
            add_product_url, headers=headers, json=outage_data, timeout=30
        )
        assert (
    ...
    try:
        response_restored = requests.post(
            add_product_url, headers=headers, json=restored_data, timeout=30
        )
